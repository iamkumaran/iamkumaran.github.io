//http://jsfiddle.net/headwinds/NrFyv/
...
(function() {
	...
	Physics.prototype.step = function(dt) {
		...
		if(this.debugDraw) {
			...
			/* var pattern = this.context.createPattern(player.balls, 'repeat');
			...
			//walls.top.body.SetPosition(posX,0)
			
			
			//var wt = walls.top;
			//wt.details.width = posX==0 ? wt.details.width : posX;
				//walls.top.SetUserData(wt);
	...
	Physics.prototype.collision = function() {
		...
		this.listener.BeginContact = function(contact,impulse) {
			//console.log(contact.GetFixtureA().GetBody().GetUserData().details.userData.name);
...
//console.log(physics.element.height)
		...
		// Create some walls
		walls.left = new Body(physics, { color: "red", type: "static", x: 0, y: 0, height: physics.element.height,  width: 0.5, userData:{name:'wall'} });
		//walls.right = new Body(physics, { color: "red", type: "static", x:51, y: 0, height: 50,  width: 0.5, userData:{name:'wall'}});
		walls.top = new Body(physics, { color: "red", type: "static", x: 0, y: 0, height: 0.5, width: beams.length*5, userData:{name:'wall'} });
		walls.bottom = new Body(physics, { color: "red", type: "static", x: 0, y:inner_height, height: 0.5, width: beams.length*5, userData:{name:'wall'} });
	...
	function setPillars(physics){
		...
		for (var i = 2; i < 3000; i++){
			...
			beams.push(new Body(physics, { image: img_pillars, type: "static", x: x, y: y, height: ht,  width: wt, userData:{name:'pillar'} }));
		}
	}
	...
	function setCoins(physics){
		...
		/* var arr = [-1,1], bool = 1, x = 0;
		for (var i = 0; i < 1000; i++){
			
			coins.push(new Body(physics, { shape: 'circle', color: 'yellow', type: "static", x: i, y: 18+(Math.sin(i)), height: 0.25,  width: 0.25, radius:0.25}));
			if(x == 10){
				bool = 0;
			}else if(x == -10){
				bool = 1;
			}
			if(arr[bool] == 1) x++; else x--;
		} */
	}
		...
		function loadImg(){
			...
			preloader[$i].onload = function(){
				...
				if($i == imgArray.length){
					...
					createWorld();
					requestAnimationFrame(gameLoop);
					setTimeout(function(){physics.pause();}, 500);
